...
int juego(char palabras[30])
{
    ...
    do
    {
        ...
        { // cuando ya se cometen el numero maximo de errores
            printf("\n GAMER OVER :(\n");
            if(puntos<0){
            printf("Su puntaje es: 0"); // si los valores quedan negativos no hay puntos
            }
            else{
            printf("Su puntaje es: %d", puntos);} //Si a pesar de perder hay puntos, se muestran aqui
            getch(); // para pausar la consola hasta presionar una tecla
        ...
        fflush(stdin);
        printf("\n presione ''S'' si desea volver a jugar o cualquier otra tecla sino es así: ");
        scanf("%s", &res);
        fflush(stdin);
    } while (res == 'S' || res == 's'); // si el caracter ingresado es S repite el juego
    printf("Precione ''N'' para volver al menu");
    scanf("%s", &res1);
    if (res1 != 'N' || res1 != 'n')
...
void nivelFacil()
{
    ...
    juego(palabras[n]); // llamando y mandando el valor a la funcion juego
}
